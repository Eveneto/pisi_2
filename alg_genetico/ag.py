...
def entrada(arquivo_entrada):
    tsp = tsplib.load(arquivo_entrada)
    return {i: coord for i, coord in enumerate(tsp.node_coords.values())}

def gerar_populacao_inicial(cidades, n_pop):
    return [shuffle(list(cidades.keys())) or list(cidades.keys()) for _ in range(n_pop)]

def calcular_distancia(caminho, cidades):
...
def avaliar_populacao(populacao, cidades):
    return [calcular_distancia(ind, cidades) for ind in populacao]
...
def evoluir(cidades, taxa_cruzamento, taxa_mutacao, tamanho_torneio, n_geracoes , populacao):
    populacao = gerar_populacao_inicial(cidades , populacao)
...
def main():
    ...
    n_geracoes = 10000
    n_populacao = 50
    melhor_rota, menor_custo = evoluir(cidades, taxa_cruzamento, taxa_mutacao, tamanho_torneio, n_geracoes , n_populacao)
